...
To finish the formulation of the model we assume indepedent normal prior distribution for each element of $\beta$, ie. $p(\beta) = \mathcal{N}(0, \sigma^2I)$.
...
Now, `fit_tailor` is a list object, where the first element, `chain` is of class "mcmc". This element can be summarized by functions provided by the [coda](https://cran.r-project.org/web/packages/coda/index.html) package.

For istnace, we can plot  daignostic sucha as traceplots.
```{r}
library(coda)
...
In order to avoid using either global variables or $\dots$ arguments, we use the function factory pattern (refs). This is a robust way to pass information to a function being
