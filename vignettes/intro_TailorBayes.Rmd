...
Tailored Bayes (TB), allows for the incorporation of misclassification costs into Bayesian modelling. In a nutshell, this is done by introducing datapoint-specific weights which are used to downweight each datapoint's likelihood contribution when fitting the model.
...
This allows more flexibility since the user can specify any number of extra arguments in the first function call. The second should always be as specified above. 
